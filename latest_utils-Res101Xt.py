...
class ResNeXt101Encoder(nn.Module):
    ...
    def __init__(self, embed_dim=512, pretrained=True):
        super().__init__()
        # 加载预训练的ResNeXt-101  weights=ResNeXt101_32X8D_Weights.DEFAULT
        self.resnext = models.resnext101_32x8d(weights=ResNeXt101_32X8D_Weights.DEFAULT)
        ...
        # 移除最后的分类层
        self.features = nn.Sequential(*list(self.resnext.children())[:-1])

        # 添加投影层，将2048维特征映射到指定维度
...
class GlobalFourierPath(nn.Module):
    """全局傅里叶路径：使用ResNeXt-101替代CLIP"""
    def __init__(self, embed_dim=512):
        super().__init__()
        self.embed_dim = embed_dim
        self.encoder = ResNeXt101Encoder(embed_dim)

    def forward(self, global_img):
        with torch.no_grad():
            features = self.encoder(global_img)
        return features


class LocalWaveletPath(nn.Module):
    """局部小波路径：使用ResNeXt-101替代CLIP"""

    def __init__(self, embed_dim, num_crops):
        super().__init__()
        self.embed_dim = embed_dim
    ...
    def forward(self, crops):
        ...
        # 合并批次和裁剪数量维度
        crops = crops.contiguous().view(-1, 3, 224, 224)  # [B*num_crops, 3, 224, 224]
        # 通过编码器提取特征
        features = self.encoder(crops)
        # 恢复批次和裁剪数量维度
        features = features.view(batch_size, self.num_crops, -1)  # [B, num_crops, embed_dim]
        return features
