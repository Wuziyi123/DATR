...
class Flickr30kDataset(torch.utils.data.Dataset):
    """Flickr30K数据集加载器（简化版）"""
    def __init__(self, image_dir, annotation_df, data_path, vocab, data_split, mode='train', num_crops=32,
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            if self.mode == 'train':  # 训练模式随机选1个字幕
                ...
                original_text = captions[selected_idx]  # 保存原始文本用于增强
            else:  # 验证/测试模式使用全部字幕
            ...
            return {
                ...
                'original_text': original_text  # 新增：返回原始文本用于增强
            }
...
# std=[0.229, 0.224, 0.225]  # Swin的标准预处理
...
def transform_random_crop(image):
    """局部图像随机裁剪变换"""
    return transforms.Compose([
        transforms.RandomResizedCrop(336, scale=(0.5, 0.9)),
