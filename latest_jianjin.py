...
class AdvancedCrossModalRetriever(nn.Module):
    ...
    def __init__(self, clip_model, trans_model=None, embed_dim=512, sim_dim=256, num_crops=5, num_semantic_tokens=8):
        ...
        self.cross_attention_weight = nn.Parameter(torch.tensor(0.01))  # 初始权重很小
        self.cross_attention_enabled = False  # 初始禁用
    ...
    def set_cross_attention_stage(self, epoch, total_epochs=60):
        """根据训练进度调整交叉注意力权重"""
        if epoch < total_epochs * 0.3:  # 前30% epoch：禁用交叉注意力
            self.cross_attention_enabled = False
            self.cross_attention_weight.data.fill_(0.01)
        elif epoch < total_epochs * 0.6:  # 30%-60% epoch：逐步引入
            self.cross_attention_enabled = True
            target_weight = 0.1 + 0.4 * (epoch - total_epochs * 0.3) / (total_epochs * 0.3)
            self.cross_attention_weight.data.fill_(target_weight)
        else:  # 后40% epoch：完全启用
            self.cross_attention_enabled = True
            self.cross_attention_weight.data.fill_(0.5)  # 最终权重0.5，保持平衡
    ...
    def forward(self, images, texts, attention_mask=None, original_texts=None):
        ...
        if self.mode == "train":
            ...
            if self.cross_attention_enabled:
                ...
                alpha = torch.sigmoid(self.cross_attention_weight)  # 限制在0-1之间
                local_text = (1 - alpha) * local_text + alpha * attn_output
            else:
                # 初始阶段：直接使用原始特征
                attn_weights = None
...
def train_retriever():
    ...
    for epoch in range(num_epochs):
        ...
        model.set_cross_attention_stage(epoch, num_epochs)
