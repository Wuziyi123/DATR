...
class CrossModalDistributionAlignment(nn.Module):
    """
    跨模态分布对齐：最优传输和分布匹配
    引用：NeurIPS 2023 "Optimal Transport for Cross-Modal Alignment"
    """
    def __init__(self, embed_dim, num_iter=5):
        super().__init__()
        self.embed_dim = embed_dim
    ...
    def sinkhorn_iterations(self, C, a, b):
        """Sinkhorn迭代求解最优传输"""
        u = torch.ones_like(a)
        v = torch.ones_like(b)

        for _ in range(self.num_iter):
            v = b / (torch.matmul(u, torch.exp(-C / self.reg) + 1e-8))
            u = a / (torch.matmul(torch.exp(-C / self.reg), v.t()) + 1e-8)

        P = u.unsqueeze(2) * torch.exp(-C / self.reg) * v.unsqueeze(1)
        return P
    ...
    def forward(self, image_feats, text_feats):
        ...
        cost_matrix = 1 - torch.matmul(image_norm, text_norm.transpose(1, 2))

        # 均匀分布
        a = torch.ones(batch_size, device=image_feats.device) / batch_size
        b = torch.ones(batch_size, device=text_feats.device) / batch_size

        # 计算传输计划
        transport_plan = self.sinkhorn_iterations(cost_matrix, a, b)

        # 对齐特征
        aligned_image = torch.matmul(transport_plan, text_feats)
        aligned_text = torch.matmul(transport_plan.transpose(1, 2), image_feats)

        # 对齐损失
        align_loss = torch.mean(torch.sum(transport_plan * cost_matrix, dim=(1, 2)))

        return aligned_image, aligned_text, align_loss
