...
class MultiScaleTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, trans_model=None, depth=12):
        ...
        self.cls_token1 = nn.Parameter(scale * torch.randn(1, 1, dim))
        ...
        self.transformer_layers = nn.ModuleList([
            trans_model.resblocks[i] for i in range(depth)
        ])
        for i in range(10):  # 前10层冻结
    ...
    def forward(self, x):
        ...
        # 添加分类token和位置编码
        if H == 28:
            cls_tokens = self.cls_token0.expand(B, -1, -1)
            x = torch.cat([cls_tokens, x], dim=1)
            x = x + self.pos_embed0[:(H * W + 1), :]
        elif H == 14:
            cls_tokens = self.cls_token1.expand(B, -1, -1)
            x = torch.cat([cls_tokens, x], dim=1)
            x = x + self.pos_embed1[:(H * W + 1), :]
        else:
            cls_tokens = self.cls_token2.expand(B, -1, -1)
            x = torch.cat([cls_tokens, x], dim=1)
            x = x + self.pos_embed2[:(H * W + 1), :]
...
class AdaptiveFusion(nn.Module):
    ...
    def __init__(self, dim, num_scales):
        ...
        # 空间特征增强（轻量级1D卷积）
        self.spatial_fuse = nn.Conv1d(num_scales, num_scales, kernel_size=3, padding=1)
        self.init_weights()
    ...
    def forward(self, features):
        ...
        # 通道注意力加权
        attn_weights = self.channel_attn(stacked.mean(dim=1))  # [B, num_scales]
        channel_fused = torch.einsum('bsd,bs->bd', stacked, attn_weights)
        # 空间交互增强
        spatial_fused = self.spatial_fuse(stacked)  # [B, num_scales, D]
        return channel_fused + spatial_fused.mean(dim=1)
...
class LocalPath(nn.Module):
    ...
    def __init__(self, dim=768, heads=8, trans_model=None, scales=[28, 14, 7]):
        ...
        # 特征融合模块
        self.fusion = AdaptiveFusion(dim, len(scales))
    ...
    def forward(self, crops):
        ...
        for i in range(N):
            ...
            orig_feat = self.image_local_proj(fused_feat)

            all_crop_features.append(orig_feat)
