...
class SimilarityComputer(nn.Module):
    ...
    def __init__(self, embed_dim, sim_dim=512, mode="train", num_self_layers=2):
    ...
    def forward(self, attention_mask, global_img, global_text, local_img, local_text):
        ...
        for i in range(len(text_lengths)):
            ...
            # === 全局相似度计算 ===
            sim_global = torch.pow(torch.sub(global_img[i].unsqueeze(0), global_text[i].unsqueeze(0)), 2)
            sim_global = l2norm(self.sim_tranglo_wg(sim_global), dim=-1)
            sim_global = sim_global.repeat(n_caption, 1).unsqueeze(1)
            global_sim_list.append(sim_global)
            sim_golo_v = torch.cat([sim_global, sim_loc_v], 1)
        ...
        sims = self.sim_enc(sim_t_list, sim_v_list)
