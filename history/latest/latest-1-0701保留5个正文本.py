...
class CrossModalPurifier(nn.Module):
    ...
    def forward(self, vis_feats, text_feats, global_vis):
        ...
        aggregated_text  = torch.mean(enhanced_text, dim=2)
...
def train_retriever():
    ...
    eval_image_ids = image_ids[200:300]  # 后续1000张评估集
    test_image_ids = image_ids[300:]  # 其余为训练集
    # 创建训练集和验证集数据框
    ...
    for epoch in range(num_epochs):
        ...
        # 保存模型
        torch.save(model.state_dict(), f"advanced_retriever_epoch{epoch + 1}.pth")

        # 验证逻辑
        val_loss = validate(model, eval_loader, device)
