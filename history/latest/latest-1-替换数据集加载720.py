...
from latest_utils import (TextEncoder, ContextEnhancer, LocalWaveletPath, init_weights,
...
class AdvancedCrossModalRetriever(nn.Module):
    ...
    def forward(self, images, texts, attention_mask=None):
        ...
        global_feat = self.image_global_proj(global_feat)
        ...
        # ===== 特征净化 =====
        # puri_text = self.feature_purifier(None, text_feats, None)
        ...
        # puri_text = F.normalize(puri_text, p=2, dim=-1)

        # 假设我们有以下特征:
        # global_feat: (B, D) 全局图像特征
        # global_text: (B, 5, D) 全局文本特征
        # local_feats: (B, num_crops, D) 局部图像特征
        # local_text: (B, seq_len, D) 局部文本特征

        # 计算相似度
        sim_matrix = None
        if self.mode == "train":
            sim_matrix = self.similarity_computer(
        ...
        #     "global_vis": global_feat,
        #     "sim_matrix": sim_matrix,  # (B, 5) 相似度矩阵
...
def train_retriever():
    ...
    train_dataset = Flickr30kDataset(image_dir, train_annotations, mode='train', rpn_proposals_file=rpn_file)
    eval_dataset = Flickr30kDataset(image_dir, eval_annotations, mode='eval')
    test_dataset = Flickr30kDataset(image_dir, test_annotations, mode='test')
    # 创建数据加载器
    dataloader = DataLoader(train_dataset, batch_size=96, shuffle=True, num_workers=4, pin_memory=True)
    ...
    # criterion = MultiContrastiveLoss()
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, batch in progress_bar:
            ...
            # 梯度裁剪（使用scaler.unscale_确保在正确的尺度上）
