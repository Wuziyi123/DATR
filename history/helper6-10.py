...
class GlobalExpert(nn.Module):
    ...
    def __init__(self, dim, expansion_ratio=4, num_heads=16):
        super().__init__()
...
class FlickrAdapter(nn.Module):
    """Flickr30k适配器（处理多文本描述）"""
    ...
    def forward(self, text_features):
        ...
        return text_emb, self.logit_scale.exp()
...
class GOAL_CLIP(nn.Module):
    def __init__(self, clip_model, device, num_local_experts=4, num_crops=5):
        ...
        self.local_proj = nn.ModuleList([
            nn.Linear(512, 256) for _ in range(num_crops)
        ])

        # 文本适配器（处理多描述）
        self.text_adapter = FlickrAdapter(clip_model)

        # 新增门控控制器（全局+5个局部）
        self.gate_controller = nn.Sequential(
            nn.Linear(256, 1),  # 每个特征独立打分
            nn.Flatten(start_dim=1)  # 将[B,6,1]压缩为[B,6]
        )
    ...
    def encode_text(self, texts):
        """处理多文本描述"""
        x = self.token_embed(texts).type(self.dtype)
        ...
        # 取EOS位置的嵌入作为文本特征
        eos_features = x[torch.arange(x.shape[0]), texts.argmax(dim=-1)]
        return eos_features @ self.text_projection
    ...
    def forward(self, images, texts):
        ...
        # 多专家特征融合
        combined_feat = self.fuse_features(global_feat, crop_feats)

        # 文本处理（每个图像对应5个描述）
        text_feats = []
        for i in range(texts.shape[1]):  # 处理每个描述
            text_feat = self.encode_text(texts[:, i])
            text_feats.append(text_feat)
        text_feats = torch.stack(text_feats, dim=1)  # [B, 5, 512]

        # 文本适配
        text_emb, logit_scale = self.text_adapter(text_feats)  # [B, 5, 256]

        # 计算相似度
        image_features = F.normalize(combined_feat, dim=-1)  # [B, 256]
        text_features = F.normalize(text_emb, dim=-1)        # [B, 5, 256]

        # 多描述相似度计算
        logits_per_image = logit_scale * torch.einsum('bd,bkd->bk', image_features, text_features)  # [B, 5]
        # logits_per_text = logit_scale * text_features @ image_features.unsqueeze(-1)
        # 文本到图像相似度计算（修正）
        text_features_flat = text_features.reshape(-1, text_features.shape[-1])  # [B*5, 256]
        logits_per_text = logit_scale * torch.matmul(
            text_features_flat,
            image_features.t()  # [256, B] 转置
        )  # [B*5, B]
    ...
    def fuse_features(self, global_feat, local_feats):
        """
        融合全局特征与多局部特征（使用pytorch_wavelets优化版）
        """
        # ===== 1. 维度对齐 =====
        global_emb = self.global_proj(global_feat)  # [B,512]->[B,256]
        local_feats = local_feats.unbind(dim=1)  # 解绑为5个张量

        # ===== 2. 小波多尺度处理 =====
        processed_features = []
        dwt_forward = DWTForward(wave='haar', mode='zero', J=1)

        for feat in [global_emb] + list(local_feats):
            # 转换为空间特征 [B, C, H, W]
            B, C = feat.shape
            H = W = int(C ** 0.5)
            spatial_feat = feat.view(B, C, H, W)

            # 使用pytorch_wavelets进行小波分解
            ll, coeffs = dwt_forward(spatial_feat)
            LH, HL, HH = coeffs[0]

            # 低频处理 (LL子带)
            ll_feat = ll.permute(0, 2, 3, 1).reshape(B, -1, C)
            ll_processed = self.global_expert(ll_feat)

            # 高频处理 (LH, HL, HH子带)
            hf_feats = []
            for hf_band in [LH, HL, HH]:
                hf_processed = self.hf_expert(hf_band)
                hf_feats.append(hf_processed)

            # 合并高频特征
            hf_combined = torch.stack(hf_feats).mean(dim=0)
            hf_processed = hf_combined.permute(0, 2, 3, 1).reshape(B, -1, C)

            # 动态门控融合
            gate_input = torch.cat([ll_processed, hf_processed], dim=1)
            global_weights = torch.sigmoid(self.gate_global(gate_input.mean(dim=1)))
            fused_feat = global_weights * ll_processed + (1 - global_weights) * hf_processed
            processed_features.append(fused_feat)

        # ===== 3. 多尺度深度特征增强 =====
        enhanced_global = self.depth_conv(processed_features[0])
        enhanced_locals = [self.depth_conv(feat) for feat in processed_features[1:]]

        # ===== 4. 空间注意力机制 =====
        gate_input = torch.stack([enhanced_global] + enhanced_locals, dim=1)
        spatial_attn = self.spatial_attention(gate_input)

        # ===== 5. 加权融合 =====
        weighted_global = spatial_attn[:, 0].unsqueeze(-1) * enhanced_global
        weighted_locals = sum(
            spatial_attn[:, i + 1].unsqueeze(-1) * emb
            for i, emb in enumerate(enhanced_locals)
        )
        return weighted_global + weighted_locals
    ...
    def multi_positive_contrastive_loss(self, logits_per_image, logits_per_text, temperature=0.07):
        ...
        输入:
            logits_per_image: [batch_size, 5] 图像到文本相似度
            logits_per_text: [batch_size*5, 1] 文本到图像相似度
        输出:
            标量损失值
        """
        ...
        # ===== 图像到文本损失（保持不变）=====
        img_targets = torch.ones(batch_size, num_captions, device=logits_per_image.device) / num_captions
        logits_img = logits_per_image / temperature
        log_max = logits_img.max(dim=-1, keepdim=True).values
        safe_logits = logits_img - log_max.detach()  # 数值平移
        img_loss = F.kl_div(
        ...
        # ===== 文本到图像损失（关键修改）=====
        # 创建目标标签 [batch_size*5]
        txt_labels = torch.repeat_interleave(
            torch.arange(batch_size, device=logits_per_text.device),
            num_captions
        )  # 形状: [batch_size*5]

        # 使用交叉熵计算（输入形状 [B*5, B]）
        # 交叉熵添加logit裁剪
        logits_txt = logits_per_text / temperature
        logits_txt = torch.clamp(logits_txt, min=-50, max=50)  # 防止溢出
        txt_loss = F.cross_entropy(logits_txt, txt_labels)
