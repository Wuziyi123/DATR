...
class GOAL_CLIP(nn.Module):
    def __init__(self, clip_model, device, paths, num_local_experts=4):
        super().__init__()
        # 初始化CLIP骨干
        ...
        self.num_points = 5  # 每张图随机生成提示点数

        # 对齐投影层
        self.global_proj = nn.Linear(512, 256)
        self.txt_proj = nn.Linear(512, 256)
        self.local_proj = nn.ModuleList([
            nn.Linear(512, 256) for _ in range(5)
        ])
    ...
    def forward(self, images, texts):
        ...
        crop_images = images[:, 1:, ::].flatten(0, 1)
        # 图像特征提取
        global_feat = self.visual(global_images.type(self.dtype))
        # 文本特征提取（支持长文本）
        text_feat = self.txt_proj(texts)
        # text_feat = None

        # 局部区域检测
        # local_regions = self.extract_local_regions(crop_images)
        local_feats = self.visual(crop_images.type(self.dtype))
        # local_feats = [self.visual(region.type(self.dtype)) for region in crop_images]

        # 多专家特征融合
        local_feats = local_feats.reshape(batch, -1, 512)
    ...
    def fuse_features(self, global_feat, local_feats):
        ...
        输出：
            combined_feat : [B, 256]     # 融合后的对比学习特征
        """
        with torch.cuda.amp.autocast(enabled=True):
            ...
            # 局部特征解绑与分治投影 [B,5,512] -> 5×[B,512] -> 5×[B,256]
            local_feats = local_feats.unbind(dim=1)  # 沿维度1解绑为5个张量
            local_embs = [proj(feat) for proj, feat in zip(self.local_proj, local_feats)]  # 分治投影

            # === 动态门控网络 ===
            # 构造门控输入 [B,6,256]（全局+5局部）
            gate_input = torch.stack([global_emb] + local_embs, dim=1)  # 多模态堆叠

            # 独立评分代替均值池化
            gate_scores = self.gate_controller(gate_input)  # [B,6]
            gate_weights = torch.softmax(gate_scores, dim=-1)  # [B,6]

            # 加权融合（保持不变）
            weighted_global = gate_weights[:, 0].unsqueeze(-1) * global_emb
            weighted_locals = sum(w.unsqueeze(-1) * emb for w, emb in zip(gate_weights[:, 1:].unbind(1), local_embs))
        ...
        with torch.no_grad():
            for batch in tqdm(dataloader):
                ...
                for i in range(len(labels)):
                    ...
                    batch_lables.append(label_embeddings)
                labels = torch.cat(batch_lables, dim=0)
