...
def adaptive_crop(
    ...
    while len(crop_boxes) < num_crops:
        # augmented_imgs.extend(processor(random_crop(img))
        crop_w = np.random.randint(int(0.05 * w), int(0.2 * w))
        crop_h = np.random.randint(int(0.05 * h), int(0.2 * h))
        x1 = np.random.randint(0, w - crop_w)
        y1 = np.random.randint(0, h - crop_h)
        crop_boxes.append((x1, y1, crop_w, crop_h))
...
def get_crop_Images(predictor, paths, processor=_transform(224), n_samples=20):
    batch_crop_imgs = []
...
if __name__ == "__main__":
    ...
    sam.eval()
    predictor = SamPredictor(sam)
    crops = get_crop_Images(predictor, ['run.jpg',])
    pass
