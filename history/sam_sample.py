...
from generate_box import dynamic_size_bboxes
...
# plt.axis('on')
# plt.show()
selected_points = grid_sampling_on_image('run.jpg',
                                         input_points, scores, grid_size=8)
# selected_points = selected_points[5:15]
# bboxes = dynamic_size_bboxe s(selected_points[:5], min_size=15, scale=0.3, img_size=(image.shape[0], image.shape[1]))
...
masks = []
scores = []
# input_labels = np.ones(len(selected_points), dtype=np.int64)  # 所有点均为正样本

# for i in range(len(selected_points)):
#     a = selected_points[i]
#     mask, score, logit = predictor.predict(
#         point_coords=np.array([selected_points[i]]),
...
# 使用示例
img = cv2.imread('run.jpg')  # OpenCV读取图像
# 生成裁剪区域
crop_regions = adaptive_crop(img, mask)
