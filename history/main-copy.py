...
# pydevd_pycharm.settrace('127.0.0.1', port=3931, stdoutToServer=True, stderrToServer=True)
...
def main(
    ...
    def custom_loader(path: str) -> torch.Tensor:
        ...
        # crop_imgs = get_crop_Images(predictor, path)

        # for i, (x, y, w, h) in enumerate(crop_imgs):
        #     crop = image[y:y + h, x:x + w]
        #     crop_imgs = processor(Image.fromarray(crop))
        #     augmented_imgs.append(crop_imgs)
        ...
        return torch.unsqueeze(processor(img), 0)
    ...
    max_size = precomputed_features.size(1)   # 50000, nsamples, 513
    image_features = image_features.to(device) # 50000, 512
    ...
    with torch.no_grad():
        ...
        for method in methods:
            ...
            # Baseline   logits = (50000, 1000) 5万张评估图片和1000个类别的相似度
            ...
            if method_name == "ours":
                ...
                for i in range(hparams["n_run"]):
                    ...
                    # precomputed_features = 50000, nsamples, 513
                    sampled_features = precomputed_features[:, random_indices, :]

                    patch_embeds = sampled_features[:, :, :-1] # 50000, 60, 512
                    patch_weights = sampled_features[:, :, -1] # 50000, 60, 1
                    ...
                    # Ours: [B, D] @ [C, D].T -> (B, C)
                    logits = patch_embeds.half() @ zeroshot_weights
                    acc_list.append(
