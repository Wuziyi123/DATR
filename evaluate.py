...
def validate(model, val_loader, device):
    ...
    # ===== 添加推理时间测算 =====
    inference_start_time = torch.cuda.Event(enable_timing=True)
    inference_end_time = torch.cuda.Event(enable_timing=True)

    torch.cuda.synchronize()  # 确保CUDA操作同步
    inference_start_time.record()
    ...
    # 记录推理结束时间
    inference_end_time.record()
    torch.cuda.synchronize()
    inference_time = inference_start_time.elapsed_time(inference_end_time) / 1000.0  # 转换为秒
...
def shard_attn_scores(model, img_embs, local_img_embs, cap_embs, local_cap_embs, attention_masks, device,
    ...
    for i in range(n_im_shard):
        ...
        for j in range(n_cap_shard):
            ...
            # 计算相似度（传递所有必要特征）
            with torch.no_grad():
                sim_t_list, sim_v_list = model.similarity_computer(
                    attn_batch,
                    im_batch,
                    cap_batch,
                    local_im_batch,
