...
def get_points(path):
    ...
    high_saliency = np.where(saliency_map > 150)
...
def adaptive_crop(img, mask, num_crops=20):
    ...
    region_boxes = [tuple(region.bbox) for region in regions]  # (min_y, min_x, max_y, max_x)

    while len(crops) < num_crops:
        scale = np.random.choice(scales)
        crop_w, crop_h = int(w * scale), int(h * scale)
        valid_width = w - crop_w
        valid_height = h - crop_h
        if valid_width <= 0 or valid_height <= 0:
            continue

        # 优先在连通域内生成候选框
        if region_boxes:
            region = region_boxes[np.random.randint(0, len(region_boxes))]
            min_x, min_y, max_x, max_y = region[1], region[0], region[3], region[2]
            x = np.random.randint(max(min_x - crop_w, 0), min(max_x, w - crop_w))
            y = np.random.randint(max(min_y - crop_h, 0), min(max_y, h - crop_h))
        else:
            x = np.random.randint(0, valid_width)
            y = np.random.randint(0, valid_height)

        # 检查区域有效性
        region = labeled_mask[y:y + crop_h, x:x + crop_w]
        if np.any(region > 0):
            current_box = (x, y, x + crop_w, y + crop_h)
            if not is_overlapping_optimized(current_box, used_boxes):
                crops.append((x, y, crop_w, crop_h))
                used_boxes.append(current_box)
...
def is_overlapping_optimized(new_box, existing_boxes, iou_thresh=0.3):
    # 使用NumPy向量化计算
    if not existing_boxes:
        return False
    existing_boxes_arr = np.array(existing_boxes)
    new_area = (new_box[2] - new_box[0]) * (new_box[3] - new_box[1])
    existing_areas = (existing_boxes_arr[:, 2] - existing_boxes_arr[:, 0]) * (
                existing_boxes_arr[:, 3] - existing_boxes_arr[:, 1])

    xi1 = np.maximum(new_box[0], existing_boxes_arr[:, 0])
    yi1 = np.maximum(new_box[1], existing_boxes_arr[:, 1])
    xi2 = np.minimum(new_box[2], existing_boxes_arr[:, 2])
    yi2 = np.minimum(new_box[3], existing_boxes_arr[:, 3])

    inter_area = np.maximum(0, xi2 - xi1) * np.maximum(0, yi2 - yi1)
    iou = inter_area / (new_area + existing_areas - inter_area)
    return np.any(iou > iou_thresh)
#*************************************************************************************

def get_crop_Images(path="data/imagenet/val/n04418357/ILSVRC2012_val_00036021.JPEG",):
    device = "cuda"
    image = cv2.imread(path)
    image_plt = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
    ...
    predictor.model.half()  # 将模型转换为 FP16
